...
namespace mkaul::graphics {
	...
	void GdiplusGraphics::draw_rectangle(
		...
		if (drawing_ and p_graphics_buffer_) {
			...
			if (0 < round_radius_x or 0 < round_radius_y) {
				...
				// 半径のリミット
				round_radius_x = std::min(round_radius_x, rect.width() * 0.5f);
				round_radius_y = std::min(round_radius_y, rect.height() * 0.5f);
	...
	void GdiplusGraphics::draw_bitmap(
		...
		p_graphics_buffer_->DrawImage(
			...
			0.f, 0.f, (Gdiplus::REAL)bitmap->width(), (Gdiplus::REAL)bitmap->height(),
